"""Kaggle kernel: Joint Target Redesign Stage A RUN C (full-train + binding margin).
...
Config: configs/unified_stage_c_margin.yaml (checkpoint_subdir unified_stage_c_margin).
...
cfg_path = REPO / "configs" / "unified_stage_c_margin.yaml"
...
ckpt_dir = REPO / "checkpoints" / "unified_stage_c_margin"
...
manifest = {
    ...
    "config": "configs/unified_stage_c_margin.yaml",
    "stage": "A-run-C-margin",
    "active_loss_terms": [
        "L_JEPA raw unnormalized (lambda_raw=3.0)",
        "VICReg var (lambda_var=5.0)",
        "VICReg cov (lambda_cov=1.0)",
        "goal margin BINDING (lambda_goal=2.0, margin=0.05)",
    ],
